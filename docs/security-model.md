...
| `webRequest` | Can observe URL + method metadata for XHR/fetch requests made by the active page | Used only as an in-memory per-tab buffer for repeated-click API shortcut hints. Request bodies are not captured. |
