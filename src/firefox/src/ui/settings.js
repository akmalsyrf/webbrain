...
async function requestProfileSyncDataConsent() { const permissions = await browser.permissions.getAll(); if (!Object.hasOwn(permissions, 'data_collection')) return window.confirm('Enable encrypted Cloud Sync? WebBrain will transmit an end-to-end encrypted vault containing your memory, profile autofill, and supported provider configuration to WebBrain Cloud. Chat history and OAuth tokens are excluded.'); return browser.permissions.request({ data_collection: ['personallyIdentifyingInfo', 'authenticationInfo', 'personalCommunications', 'websiteContent', 'technicalAndInteraction'] }); }
